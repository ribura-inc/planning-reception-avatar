...
リモートPCにて、接続開始前にGoogle Meetの利用に必要な事前条件を確認する機能を提供する。

- Googleアカウントへのログイン状態確認
- 必要なChrome拡張機能のインストール確認
- 各確認項目で問題がある場合は、該当ページを開いて設定を促す
...
2. （任意）プレチェック機能で事前準備を確認
3. フロントPCを選択し「接続を開始」→ 接続準備中 → 接続中（Meet生成・送信・ブラウザ起動）
4. セッション中は「セッション終了」のみ操作可能
5. セッション終了後は自動的に待機状態へ復帰
