...
class PlatformUtils:
    ...
    def get_vtube_studio_paths() -> list[str]:
        ...
        if current_platform == Platform.WINDOWS:
            return [
                ...
                os.path.expandvars(r"%PROGRAMFILES(X86)%\VTube Studio\VTube Studio.exe"),
            ]
        elif current_platform == Platform.MACOS:
            return [
    ...
    def launch_application(app_path: str) -> bool:
        ...
        try:
            if current_platform == Platform.WINDOWS:
                ...
                logger.error(f"Unsupported platform for launching applications: {current_platform}")
                return False
    ...
    @staticmethod
    def get_profile_directory(app_name: str) -> Path:
        """アプリケーション用のプロファイルディレクトリを取得"""
        current_platform = PlatformUtils.get_platform()
        ...
        profile_dir = base_dir / app_name
        profile_dir.mkdir(parents=True, exist_ok=True)
        return profile_dir

    @staticmethod
    def get_chrome_driver_path() -> str | None:
        """ChromeDriverのパスを取得（自動的にダウンロードされる場合は不要）"""
        # Selenium 4.6以降はChromeDriverを自動管理するため、通常は不要
        return None
