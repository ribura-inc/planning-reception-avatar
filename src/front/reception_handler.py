...
class ReceptionHandler:
    ...
    def _handle_meet_url(self, data: dict[str, Any]) -> None:
        ...
        self._update_gui(ConnectionStatus.CONNECTING.value, f"Meet URLを受信: {meet_url}", "remote-pc")
    ...
    def _join_meeting(self, meet_url: str) -> None:
        ...
        try:
            ...
            if self.meet_participant.join_meeting(meet_url):
                ...
                self._update_gui(ConnectionStatus.CONNECTED.value, "Meetへの参加が完了しました")
            else:
                logger.error("Meetへの参加に失敗しました")
                self._update_gui(ConnectionStatus.ERROR.value, "Meetへの参加に失敗しました")
    ...
    def _end_session(self) -> None:
        ...
        try:
            ...
            self._update_gui(ConnectionStatus.WAITING.value, "セッション終了処理が完了しました")
    ...
    def start_reception(self) -> bool:
        ...
        try:
            ...
            self._update_gui(ConnectionStatus.WAITING.value, f"サーバーを開始しました (ポート: {self.port})")
